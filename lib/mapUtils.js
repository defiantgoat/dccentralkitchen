...
export function findDefaultStore(stores) {
  console.log('find default store ****************************');
  // console.log(stores);
  console.log('defaultStoreId', RecordIds.defaultStoreId);
  // console.log(RecordIds);
  console.log('store.length', stores.length);
  stores.forEach((item) => console.log(item.id));
  const defaultStore = stores.find((store) => {
    return store.id === RecordIds.defaultStoreId;
  });

  console.log('RecordIds.defaultStoreId', RecordIds.defaultStoreId);
...
export function useFilteredStores(stores, searchStr, filters = null) {
  ...
  }, [stores, filters, searchStr]); // eslint-disable-line
