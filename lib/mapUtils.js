...
function computeStoreOpen(hours) {
  try {
    ...
    console.log(todaysDayIndex, hoursPerDayDict);
 ...
 */
export function useStores(currentLocation) {
      ...
      Promise.all(
        storeData.map(async (store) => {
          try {
            ...
            const distance = findStoreDistance(currentLocation, currStore);
            const [closestZip] = await geo2zip(currStore);
            currStore.zip = closestZip;
            currStore.distance = distance;
          } catch (error) {
